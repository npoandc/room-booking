-- Migration 005: add optional notes field to bookings

ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS notes text;

-- Update create_booking to accept notes
...
BEGIN
  ...
  BEGIN
    INSERT INTO bookings (room, title, booked_by, starts_at, ends_at, notes)
    VALUES (p_room, TRIM(p_title), TRIM(p_booked_by), p_starts_at, p_ends_at, NULLIF(TRIM(COALESCE(p_notes,'')),'' ))
...
-- Update create_recurring_booking to accept notes
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      INSERT INTO bookings (room, title, booked_by, starts_at, ends_at, series_id, notes)
...
-- Update create_monthly_booking to accept notes
  ...
  FOR i IN 0..23 LOOP
    ...
    IF v_candidate IS NOT NULL
      ...
      BEGIN
        INSERT INTO bookings (room, title, booked_by, starts_at, ends_at, series_id, notes)
        VALUES (p_room, TRIM(p_title), TRIM(p_booked_by),
                v_start_ts, v_start_ts + v_duration, v_series, v_notes);
...
-- Update change_booking to accept notes
...
GRANT EXECUTE ON FUNCTION public.create_booking(text,text,text,timestamptz,timestamptz,text) TO anon;
GRANT EXECUTE ON FUNCTION public.change_booking(uuid,text,text,text,timestamptz,timestamptz,text,text) TO anon;
GRANT EXECUTE ON FUNCTION public.create_recurring_booking(text,text,text,timestamptz,timestamptz,int,date,text) TO anon;
GRANT EXECUTE ON FUNCTION public.create_monthly_booking(text,text,text,timestamptz,timestamptz,int,int,date,text) TO anon;
